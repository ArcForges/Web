...
The candidate verifier reconstructs the report from independent source/lock/CI inputs and rejects altered build/axis values even when both outer receipts are resealed. CI source/attempt guards reject dirty, unrelated or incomplete identities. Three actual browsers fetch and compare the sealed asset, first from the local candidate and then from the real Cloudflare domain. Public delivery also verifies its exact bytes with every other asset. No runtime reads a verifier's environment to fabricate its own identity.
